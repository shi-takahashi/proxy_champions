...
// ────────────────────────────────────────────────────────────
// M6 大会（勝点・決勝進出数・昇降格枠）※すべて仮値・企画書5章
//   予選（総当たりリーグ）の勝点で順位 → 上位が決勝トーナメント／上下ディビジョンへ移動。
// ────────────────────────────────────────────────────────────
export const TOURNAMENT = {
  pointsWin: 3, // 勝ち（サッカー式・引き分けに差をつける）
  pointsDraw: 1,
  pointsLoss: 0,
  bracketSize: 4, // 予選上位この人数が決勝トーナメント（2の冪へ丸める）
  promoteCount: 2, // リーグ上位が昇格（企画書5.2 J1/J2 ラダー）
  relegateCount: 2, // リーグ下位が降格
} as const;
